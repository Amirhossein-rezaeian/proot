	...
	if (key != IPC_PRIVATE) {
		...
		while (true) {
			...
			if (symlink(num_buffer, symlink_path) != 0) {
				DBG("%s: symlink failed %s -> %s", __PRETTY_FUNCTION__, symlink_path, num_buffer);
			} else {
				DBG("%s: symlink created %s -> %s", __PRETTY_FUNCTION__, symlink_path, num_buffer);
			       	break;
			}
		}
	}
...
static int handle_sysexit_end(Tracee *tracee, RegVersion stage)
{
	...
	case PR_shmget: {
		...
		int result = (word_t)shmget(key, size, flags);
		poke_reg(tracee, SYSARG_RESULT, (word_t)result);
		if (result < 0)
			return result;
	}
